            ...
            if (!that.daemonized) {
                ...
                try {
                    ...
                    if (that.noaudio) { // todo demuxer-readahead-packets=300 separate
                        cvlc = spawn("cvlc", ["-I", "rc", "--rc-fake-tty", "--no-mouse-events", "--no-keyboard-events", "--rc-host", "localhost:" + that.socketport, "--avcodec-hw", "none", "--no-audio","--loop", "--daemon"], { detached: true, stdio: "ignore" })
                    } else if (options) {
