...
class vlcdaemon {
        ...
        if (!this.daemonized) {
            ...
            if (options) {
                for (const dopt of default_options) {
                    if (!options.find((f) => f === dopt))
                        options.push(dopt);
                }
                console.log("cvlcopts0", options);
                cvlc = (0, child_process_1.spawn)("cvlc", options, { detached: true, stdio: "ignore" });
            }
            else {
                const cvlcopts = [
                    "-I",
                ...
                cvlc = (0, child_process_1.spawn)("cvlc", cvlcopts, { detached: true, stdio: "ignore" });
            }
            if (this.verbose) {
                cvlc.on("error", (data) => {
            ...
            this.player_process = net_1.default.createConnection(this.socketport, "localhost");
            const that = this;
    ...
    async loadListfromFile(playlist_path, playnow) {
        if (playlist_path && playlist_path.split(".pls").length > 1) {
            if (this.daemonized) {
                const data = await promises_1.default.readFile(playlist_path, "utf-8");
                ...
                this.playlist = tracks.map((t) => Object.assign(t, { label: t.label || (0, unicoid_1.uniqueid)(4) }));
            }
            else {
    ...
    async addTrack(track, index) {
        if (this.playlist.length > 0) {
            await writeToVlc(this.player_process, "enqueue " + track.uri + "\n");
            if (!track.label)
                track.label = (0, unicoid_1.uniqueid)(4);
        ...
        if (play_path) {
            ...
            if (this.playlist.length > 1) {
                try {
                    ...
                    await writeToVlc(this.player_process, JSON.stringify({
                        command: ["playlist-remove", "current"],
                    }) + "\r\n");
                    this.playlist = [];
                    this.playlist.push({
                        uri: play_path,
                        label: (0, unicoid_1.uniqueid)(6),
                    });
                    this.playing = true;
                    this.uri = play_path;
                    this.track = 1;
                ...
                this.playlist = [];
                this.playlist.push({ uri: play_path, label: (0, unicoid_1.uniqueid)(6) });
                this.playing = true;
                this.uri = play_path;
                this.track = 1;
            }
            else {
                await writeToVlc(this.player_process, JSON.stringify({ command: ["loadfile", play_path] }) + "\r\n");
                this.playlist.push({ uri: play_path, label: (0, unicoid_1.uniqueid)(6) });
                this.playing = true;
                this.uri = play_path;
                this.track = 1;
            ...
            await writeToVlc(this.player_process, JSON.stringify({ command: ["play"] }) + "\r\n");
            this.playing = true;
            if (!this.track)
                this.track = 1;
            if (!this.uri)
